    ...
    if (items.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-32 text-center gap-6">
                ...
                <Link
                    href="/"
                    className="bg-indigo-600 hover:bg-indigo-500 hover:shadow-lg hover:shadow-indigo-200 cursor-pointer text-white font-semibold px-6 py-3 rounded-xl text-sm transition-all active:scale-[0.98]"
                >
                    Ver catálogo
                </Link>
            </div>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
                ...
                {items.map((item) => (
                    <div key={item.cartKey} className="flex items-center gap-4 px-5 py-4">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-gray-400 font-mono uppercase tracking-wider mt-0.5">#{item.sku}</p>
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-1.5 shrink-0">
                            <div className="flex items-center gap-1.5">
                                ...
                                <button
                                    onClick={() => updateQuantity(item.cartKey, item.quantity + 1)}
                                    className="w-8 h-8 rounded-lg bg-indigo-600 hover:bg-indigo-500 flex items-center justify-center text-white transition-colors cursor-pointer"
                                >
                                    <Plus size={12} />
                                </button>
                            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4 shadow-sm">
                ...
                <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 hover:shadow-lg hover:shadow-indigo-200 cursor-pointer text-white font-semibold py-3.5 rounded-xl text-sm transition-all active:scale-[0.98]"
                >
                    Confirmar pedido
                    <ArrowRight size={15} />
                </button>
