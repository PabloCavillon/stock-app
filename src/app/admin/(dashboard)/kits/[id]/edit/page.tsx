import { getKit, getKits } from "@/actions/kits";
...
export default async function EditKitPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [kit, products, kits] = await Promise.all([getKit(id), getProducts(), getKits()]);
    ...
    return (
        <div className="max-w-3xl mx-auto p-4 md:p-12 space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-zinc-500 mt-1 font-light italic">
                            Modificando: <span className="text-zinc-900 font-medium not-italic">{kit.name}</span>
                        </p>
                    </div>
                </div>
            </header>
            ...
            <section className="bg-white p-6 md:p-12 rounded-3xl border border-zinc-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                <KitForm kit={kit} products={products} kits={kits} />
            </section>
        </div>
    );
}
