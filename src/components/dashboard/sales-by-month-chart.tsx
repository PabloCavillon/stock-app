            ...
            {data.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={240}>
                    <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <YAxis tick={{ fontSize: 11, fill: "#a1a1aa", fontWeight: 600 }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v.toLocaleString()}`} />
                        <Tooltip cursor={{ stroke: '#e4e4e7', strokeWidth: 1 }} contentStyle={{ fontSize: '12px', fontWeight: 'bold', borderRadius: '16px', border: "1px solid #f4f4f5", boxShadow: "0 10px 15px -3px rgb(0 0 0 / 0.05)", padding: '10px 12px', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#18181b' }} />
                        <Area type="monotone" dataKey="total" stroke="#18181b" strokeWidth={2.5} fillOpacity={1} fill="url(#salesGradient)" animationDuration={1500} />
                    </AreaChart>
                </ResponsiveContainer>
            )}
        </div>
